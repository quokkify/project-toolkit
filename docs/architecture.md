...
9. **Repository validation is a fail-closed DAG.** A reusable selector sends trusted same-repository PRs (including Renovate) and trusted default-branch work to an available internal self-hosted runner, while forks, untrusted authors, unknown strategies, and discovery failures fall back to `ubuntu-latest`. Shared policy, template, YAML/JSON, Renovate, actionlint, ShellCheck, and whitespace checks run once in `lint`; only after that gate succeeds do independent Python, Node.js, and Java fixture jobs run in parallel with their own toolchains. A final aggregate verifies every required stage, while CodeQL remains a separate branch and status. `python scripts/validate.py` remains the canonical local aggregate.
