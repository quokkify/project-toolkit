...
9. **Repository validation is split by failure domain.** Shared policy, template, YAML/JSON, Renovate, actionlint, ShellCheck, and whitespace checks run once in `lint`; executable Python, Node.js, and Java fixtures run in separate jobs with only their own toolchain. `python scripts/validate.py` remains the canonical local aggregate.
