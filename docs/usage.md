...
- `renovate.json` extends the selected organization presets when `renovate` is enabled.

The `Update Copier-managed repositories` workflow runs daily in `quokkify/project-toolkit`. It discovers non-archived, non-fork organization repositories containing `.copier-answers.yml`, accepts only answers whose `_src_path` resolves to `quokkify/project-toolkit`, runs `copier update` against the latest released template tag, and opens or refreshes `chore: update shared project template` pull requests when rendering changes. Existing project changes are preserved by Copier's update algorithm; `.rej` conflicts fail that repository loudly instead of opening a partial PR. The deterministic `automation/copier-template-update` branch is automation-owned and may be force-updated with an exact lease, so maintainers should not add manual commits to it.

Cross-repository writes require a dedicated repository or organization Actions secret named `COPIER_FLEET_TOKEN`. Use a fine-grained token or GitHub App installation token limited to the Quokkify repositories with **Contents: read and write**, **Pull requests: read and write**, and metadata/organization read access. The normal `GITHUB_TOKEN` is used only for manual dry-runs and cannot write to sibling repositories. A manual dispatch defaults to dry-run, can target one `owner/repository`, and can optionally test an explicit template ref. Scheduled runs intentionally follow the latest release tag rather than unreleased `main`, so a template change propagates only after Release Please publishes it.
