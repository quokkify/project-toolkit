...
Generated Renovate configuration disables the built-in `copier` manager. Direct dependency updates rewrite only `_commit`, may reserialize unrelated YAML lists, and leave `toolkit_version` plus generated workflows stale. Use the fleet updater for atomic Copier changes; Renovate remains responsible for released workflow, action, and ordinary package references.
