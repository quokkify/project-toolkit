...
`allure-report` delegates exactly once to the immutable standalone [`quokkify/allure-report-action`](https://github.com/quokkify/allure-report-action) `v0.1.3` release (commit `17ebbb05b6bf4f256a4bc5a256fbfc6f37783126`). The standalone implementation reads an already-merged results directory, writes badges and the PR summary, optionally exports test-pyramid files, optionally publishes the HTML report, and finally creates or updates one marked comment. The `module-environment-label` input defaults to `module`, matching the standalone v0.1.3 multi-module environment contract. Tests without `epic` metadata remain in overall totals: the preserved fallback classifies Playwright as `E2E`, while otherwise unclassified results appear under `Other`. Public and private repositories use the same explicit `github-token` input and repository context; callers grant `pull-requests: write`, plus `contents: write` only when Pages publishing is enabled. Pages publishing is off by default so private repositories can still generate and comment the summary without a public Pages dependency.
...
Older `.copier-answers.yml` files have no Allure answer. Fleet audit reports them as `unknown`, not `disabled`; once configured, the workflow and config must both exist for the feature to be `enabled`.
