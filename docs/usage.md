...
jobs:
  backend:
    uses: quokkify/project-toolkit/.github/workflows/python-ci.yml@v2.8.2
    with:
...
```console
copier copy https://github.com/quokkify/project-toolkit.git my-project --vcs-ref v2.8.2 --trust
cd my-project
copier update --trust
```
...
The template accepts an optional YAML list of `python`, `node`, and `java` components plus Docker, Allure, Release Please, and Renovate booleans. Leave `components` empty when the repository owns custom language-specific validation; the shared template-contract validation, CodeQL Actions analysis, and Gitleaks still remain present. Generated projects use Renovate presets from `quokkify/renovate-presets` at the released `v1.0.1` tag by default. Set both `renovate_config_repository` and `renovate_config_ref` when a project should consume another GitHub `owner/repo` and its exact released SemVer tag; custom repositories deliberately have no inherited Quokkify ref, so unattended migration fails instead of guessing a possibly nonexistent tag. Set `renovate_presets` to a non-empty YAML list of selected preset names, preserving the desired extends order. Supported names map to paths as follows: `default` -> `presets/base`, `python` -> `presets/python/default`, `javascript` -> `presets/npm/default`, `java` -> `presets/gradle/default`, `docker` -> `presets/docker/default`, and `github-actions` -> `presets/github-actions/default`. New projects infer `default`, `github-actions`, and the language/Docker presets selected by `components` and `docker`. Commit `.copier-answers.yml`; it is the update contract. Review generated pull requests before merging them.

## Copier Allure reporting

Set `allure_report: true` to generate `.github/allure/allurerc.mjs`, `.github/allure/safe_extract.py`, and `.github/workflows/allure-report.yml`. Each generated component job uploads one required, uniquely named `allure-results-<type>-<index>` artifact. The default source is `<component path>/allure-results`; set an optional safe relative `allure_results_path` on a component for layouts such as Gradle's `build/allure-results` or Maven's `target/allure-results`. Configure the project's test adapter to write there—for example `pytest --alluredir=allure-results`, an Allure-enabled Java test task, or the selected Node test framework's Allure reporter. Missing required results fail the component job. The template deliberately does not modify dependency manifests or replace project-specific test commands.

The follow-up resolves the current open PR through GitHub's API, requires its repository and head SHA to match the exact source run, validates an allowlist of expected non-expired artifacts, and suppresses stale runs. Artifact download, collision-safe bounded merging, and Allure generation happen in a read-only job. Compressed bytes are streamed into `runner.temp`; central-directory, path, type, count, per-file, and aggregate expanded-size limits are verified before any archive member is extracted. Extraction also stays under `runner.temp`; only fully validated regular files are then copied into a symlink-checked isolated workspace directory. The Pages job applies the same bounded pre-extraction gate to the generated report artifact before publication. A separate narrowly privileged job constructs a static trusted comment with links to the generated HTML artifact and source run—it never executes or posts Markdown supplied by the PR artifacts. Fork and Dependabot PRs receive that safe comment but never Pages publication.
...
Older `.copier-answers.yml` files have no Allure answer. Fleet audit reports them as `unknown`, not `disabled`; once configured, the workflow, config, and bounded extractor must all exist for the feature to be `enabled`.
...
New Copier-generated projects extend selected presets from `quokkify/renovate-presets` at the exact released `renovate_config_ref` tag (`v1.0.1` by default for that repository). Project teams using another `renovate_config_repository` must also provide that repository's own released SemVer ref; legacy custom-repository updates fail closed when the ref is missing. The `renovate_presets` answer uses user-facing names: `default` maps to `//presets/base`, `python` to `//presets/python/default`, `javascript` to `//presets/npm/default`, `java` to `//presets/gradle/default`, `docker` to `//presets/docker/default`, and `github-actions` to `//presets/github-actions/default`.

Generated Renovate configuration disables the built-in `copier` manager so fleet automation remains the single Copier rollout owner. The fleet updater applies the template, preserves existing answer-file formatting when semantics are unchanged, canonicalizes the template source, rejects `.rej` conflicts, and opens a dedicated reviewable PR. Generated validation then checks synchronized version answers and references before merge. Renovate remains responsible for released workflow, action, and ordinary package references.
