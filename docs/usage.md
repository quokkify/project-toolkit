...
The `Audit Copier-managed repositories` workflow is manual-only. A CODEOWNER can start it from the Actions UI or with `gh workflow run copier-fleet-update.yml --repo quokkify/project-toolkit`; the API call is authenticated by the caller's current `gh` session. The workflow discovers non-archived, non-fork organization repositories containing `.copier-answers.yml`, accepts only answers whose `_src_path` resolves to `quokkify/project-toolkit`, and performs a read-only `copier update` preview against the latest released template tag. It uses the repository-scoped `GITHUB_TOKEN`, stores no cross-repository credential, and its latest result drives the README badge.
